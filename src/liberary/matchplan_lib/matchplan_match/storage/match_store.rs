...
pub async fn update_match(match_info: Match, season_name: String, pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    let query_path = "src/liberary/matchplan/storage/queries/store_match.sql";
    let query = build_query(query_path, vec![
        build_query::ArgumentType::Int(match_info.p1.id as i64),
        build_query::ArgumentType::Int(match_info.p2.id as i64),
