...
pub async fn upload_accounts(appstate: &AppState) -> Result<(), String> {

    println!("{}", "Received command to catch up with match requests".magenta());

    let accounts_to_upload = match StorageMod::read_accounts() {
        Ok(accounts) => accounts,
        Err(e) => return Err(format!("Error while getting accounts: {}", e))
    };
    ...
    for (_key, account) in accounts_to_upload.iter() {
        match store_account(account.clone(), appstate.pool.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(format!("Error uploading account: {}", e.to_string()))
            }
        }
    }
    Ok(())
